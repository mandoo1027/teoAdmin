<template>
  <div class="tit_cnt_btn">
    <p class="tit_cnt" v-if="computeShowOption.isShowCnt">
      조회 건 수 <b>{{ totalCount }}</b
      >건
    </p>
    <div>
      <v-btn
        v-if="computeShowOption.isShowAdd"
        class="vbtn black"
        size="small"
        @click="setData('add')"
        >추가</v-btn
      >
      <v-btn
        v-if="computeShowOption.isShowDel"
        class="vbtn black"
        size="small"
        @click="setData('del')"
        >삭제</v-btn
      >
      <v-btn v-if="computeShowOption.isShowSave" class="vbtn" size="small" @click="saveData"
        >저장</v-btn
      >
      <CommonComSelectBox
        v-if="computeShowOption.isShowPageSize"
        v-model="comboPageSize"
        :items="sltitems"
        :isDefault="false"
        class="s_basics more none_details"
...
const props = defineProps({
  ...
  isShowPaging: { type: Boolean, default: false }, //페이징 영역 노출 여부
  style: { type: Object, default: { height: '553px' } },
...
const comboPageSize = ref(10) //페이지 사이즈
...
const computeShowOption = computed(() => {
  return {
    ...
    isShowSave: props.showOptions.isShowSave ?? false, //저장 버튼
